...
import Stats from '../components/Stats'; // Importe o novo componente

// Importações do Firebase para a lógica do "like"
...
function Home() {
  ...
  // Estado para controlar se o usuário já curtiu
  const [hasLiked, setHasLiked] = useState(false);

  // Verifica no localStorage se o like já foi dado
  useEffect(() => {
  ...
  const handleLike = async () => {
    if (!hasLiked) {
      try {
        ...
        // Incrementa o campo 'likes' em 1 no Firestore
        await updateDoc(counterRef, {
          likes: increment(1)
        });
        // Marca no localStorage que o usuário já curtiu
        localStorage.setItem('portfolio-liked', 'true');
        setHasLiked(true);
      } catch (error) {
        console.error("Erro ao incrementar like:", error);
      }
    }
  };
  ...
  return (
    <section className="min-h-screen flex flex-col items-center justify-center text-center p-4">
      <div className="max-w-4xl w-full">
        {/* ... (Toda a parte de foto, nome e TypeAnimation continua a mesma) ... */}
        ...
        <div className="opacity-0 animate-fade-in-up flex flex-col sm:flex-row items-center justify-center gap-4" style={{ animationDelay: '900ms' }}>
          ...
          <button
            onClick={handleLike}
            disabled={hasLiked}
            className="inline-flex items-center justify-center gap-2 bg-[var(--color-bg-secondary)] border border-[var(--color-border)] font-bold py-3 px-8 rounded-full text-lg transition-transform transform hover:scale-105 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
          >
