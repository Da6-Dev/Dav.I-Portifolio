    ...
    useEffect(() => {
        const fetchProjectData = async () => {
            try {
                ...
                // 2. Lógica de busca de README multilíngue
                const lang = i18n.language; // ex: 'en', 'pt'
                const translatedReadmeUrl = `https://raw.githubusercontent.com/${GITHUB_USERNAME}/${projectName}/${repoData.default_branch}/README.${lang}.md`;
                const defaultReadmeUrl = `https://raw.githubusercontent.com/${GITHUB_USERNAME}/${projectName}/${repoData.default_branch}/README.md`;

                // Tenta buscar a versão traduzida primeiro
                const translatedResponse = await fetch(translatedReadmeUrl);

                if (translatedResponse.ok) {
                    const readmeText = await translatedResponse.text();
                    setReadme(readmeText);
                } else {
                    // Se falhar, busca a versão padrão
                    const defaultResponse = await fetch(defaultReadmeUrl);
                    if (defaultResponse.ok) {
                        const readmeText = await defaultResponse.text();
                        setReadme(readmeText);
                    } else {
        ...
        fetchProjectData();
    }, [projectName, i18n.language]); // 3. Adicione i18n.language como dependência
